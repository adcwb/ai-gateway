...
| Rule-based PII engine + pluggable guardrail chain + external checker (gRPC) + audit-body encryption | ✅ | opt-in per-policy chain (`checker_chain`: `pii_rules`/`prompt_injection`/`topic_fence`/`external`), both non-streaming **and streaming** outbound scanning (log/terminate-only for streams), `audit.encrypt_bodies` AES-GCM (D06). LLM-judge escalation and `topic_fence` embedding-similarity mode remain 🔴 |
