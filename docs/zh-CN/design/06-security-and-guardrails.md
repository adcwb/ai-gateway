...
### 第四轮：修复——`block` 策略曾可能让一轮对话永久死锁

这是一个真实的 bug，不是范围取舍：两条入站引擎路径（`applyPIIPolicy` 里的旧版 `scanPII` 调用，以及 `runInboundChain` 的 `chain.Run`）此前都是把**整个原始请求体当作一整块文本**来扫描——`messages` 里的每一条消息都会被扫到，而不只是调用方最新发来的那一条。遵循 OpenAI 惯例的客户端每次请求都会把完整消息历史重新带上，于是一旦某条 `block` 动作的策略在对话中的任意位置命中过一次（比如第 1 轮里用户输入的手机号），同一段历史文本会在之后每一轮都重新命中——哪怕用户这一轮的新输入完全干净，请求依然会被永久拒绝，除非用户抛弃当前对话重开一个新的。

- **修复：`internal/biz/pii_turn.go` 的 `currentTurnText`** 会单独取出最后一条 `role:"user"` 消息自己的文本（同时处理纯字符串和多模态的 block 数组两种 `content` 形态）。`applyPIIPolicy` 和 `runInboundChain` 的 block 分支现在都会先针对*仅这段文本*重新检测一遍，再决定是否真的执行拦截；如果整轮命中只出现在历史消息里，就降级为不拦截、仅记录审计的 log 动作——同时历史里的敏感内容依然会先脱敏再转发给上游，而不是原样放行。
- **额外开销只发生在本就罕见的"即将拦截"路径上。** 原本那一次覆盖全文的扫描/chain 执行完全不变，仍然是每次请求都要跑一次；范围收窄后的复核只在这一次扫描已经命中了拦截级别的结果时才会追加执行——绝大多数"什么都没命中"和"命中但动作是 redact/log"的请求，开销和修复前完全一样。
- **检测链路径下的脱敏完整性说明：** 由于 `policy.Action` 是整条链统一生效的（见第二轮 ADR 的 `fixedAction` 说明），一条配成 `block` 的链里每个 checker 命中时都只会报 `ActionBlock`，从不会报 `ActionRedact`——也就是说 `Chain.Run` 自己那套"命中 redact 才改写 `finalText`"的逻辑根本不会触发，而且命中 block 会立刻短路返回，链上排在后面的 checker 根本不会执行。因此降级路径改为直接从触发拦截的那条 finding 上取脱敏后的文本（`pii_rules_checker` 无论 `Action` 是什么都会填充 `Finding.Redacted`），而不是信任 `finalText`；像 `prompt_injection`/`topic_fence` 这类关键词/启发式 checker 本来就没有文本级脱敏可言，这种情况下只能把原始未脱敏文本转发出去——这和本管线在别处早已承认的"规则级、非语义级"的诚实表述是一致的，现在把这一路径也补上了文档说明。
- **同样的"客户端重发"模式还会出现在更细的一层：** 有些聊天客户端在上一条消息被拦截后，会把残留的敏感片段和用户本次的新输入拼成同一条多模态消息里的两个 block 发过来（`content: [{"type":"text","text":"<上次被拦截的内容>"},{"type":"text","text":"<新输入>"}]`——在 Cherry Studio 的重试行为中观察到）。这种情况下没有可靠的办法区分"残留片段"和"真正的新内容"，所以 `currentTurnText` 会把任何多 text-block 的当前消息一律当作历史处理：不能单凭它触发拦截。
- **非聊天类请求体不受影响。** `currentTurnText` 在识别不出 `messages` 数组时（比如 `/embeddings` 的 `input` 字段，或其他任何形态）返回 `ok=false`，两处调用点都会退回到修复前那种不区分范围、扫描全文的原始判断逻辑——这一轮改动只影响真正的多轮对话请求。
- 回归测试覆盖：`internal/biz/pii_turn_test.go`——历史消息里的命中在旧版引擎和检测链两条路径下都会降级；当前轮自己的命中在两条路径下依然会拦截；多 block 重发场景不会拦截；非聊天类请求体保持修复前原样的硬拦截行为。
