...
### Round 4: fix — a `block` policy could permanently deadlock a conversation

A real bug, not a scope decision: both inbound engines (`applyPIIPolicy`'s legacy `scanPII` call and `runInboundChain`'s `chain.Run`) scanned the **entire raw request body as one blob** — every message in `messages`, not just the caller's newest one. An OpenAI-convention client resends the full message history on every call, so once a `block`-action policy matched something anywhere in a conversation (say, a phone number typed in turn 1), that same historical text kept re-matching on every subsequent turn forever — the user's actual new input could be perfectly clean and the request was still rejected, permanently, with no way out short of starting a brand-new conversation.

- **Fix: `internal/biz/pii_turn.go`'s `currentTurnText`** isolates the latest `role:"user"` message's own text (handling both plain-string and multimodal array-of-blocks `content`). Both `applyPIIPolicy`'s block case and `runInboundChain`'s block case now re-check scoped to *only* that text before honoring a block that the (unchanged) whole-body scan found — a match that exists solely in resent history downgrades to a non-blocking, log-only audit entry instead, and the historical PII is still masked in the body actually forwarded upstream (never sent raw) rather than left untouched.
- **Cost stays bounded to the already-rare "about to block" path.** The original single whole-body scan/chain-run is still exactly what runs on every request; the scoped re-check is a second, additional call that only happens when that first pass already found a block-worthy match — the overwhelmingly common "nothing flagged" and "flagged but the action is redact/log" paths cost exactly what they did before this fix.
- **Redaction-completeness caveat in the chain path specifically:** since `policy.Action` applies chain-wide (round 2's `fixedAction` note), every checker in a `block`-configured chain always reports `ActionBlock`, never `ActionRedact` — meaning `Chain.Run`'s own `finalText` rewrite step (which only fires on a redact finding) never touches it, and the short-circuit-on-block return means later checkers in the chain never even run. The downgrade path therefore pulls the masked rewrite directly off the finding that triggered the block (`pii_rules_checker` always populates `Finding.Redacted` regardless of its `Action`) rather than trusting `finalText`; a keyword/heuristic checker like `prompt_injection`/`topic_fence` has no text-level redaction to offer in the first place, so those fall back to forwarding the unmasked original text for that specific case — the same "rule-grade, not semantic-grade" honesty this pipeline already carries elsewhere, now documented for this path too.
- **The same client-resend pattern shows up one level deeper, too:** a chat client that had a message rejected sometimes resends the leftover flagged fragment bundled together with the user's actual next input as two blocks of one multimodal message (`content: [{"type":"text","text":"<the old flagged text>"},{"type":"text","text":"<new input>"}]` — observed with Cherry Studio's retry behavior). There is no reliable way to tell "residual fragment" from "genuinely new text" apart within one message, so `currentTurnText` treats *any* multi-text-block current message the same as pure history: never eligible to justify a block on its own.
- **Non-chat bodies are untouched.** `currentTurnText`'s `ok=false` (no recognizable `messages` array — `/embeddings`' `input` field, or any other shape) makes both call sites fall back to the original, unscoped whole-body decision exactly as before this fix; this round only changes behavior for genuine multi-turn chat requests.
- Regression coverage: `internal/biz/pii_turn_test.go` — history-only matches downgrade on both the legacy and chain paths, a match in the caller's own latest turn still blocks on both paths, the multi-block resend shape never blocks, and a non-chat body keeps the pre-fix hard-block behavior verbatim.
