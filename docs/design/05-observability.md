...
**A freshly booted (or idle) instance's `/metrics` scrape looks sparse by design, not by bug.** Every `aigw_*` instrument above except `aigw_audit_queue_depth`/`aigw_concurrency_slots` is a `*Vec` (`CounterVec`/`HistogramVec`/`GaugeVec`); client_golang does not emit a time series for a label combination until `.WithLabelValues(...)` is actually called with it — `aigw_requests_total` only appears once at least one proxy request has completed, `aigw_breaker_state` once a breaker has been touched, and so on. This surprised an operator scraping a just-started instance with zero traffic and seeing only the two plain (label-less) gauges; it self-resolves once real traffic flows and isn't something this project's `NewMetrics` tries to work around (pre-registering every possible label combination up front isn't practical — provider/model/tenant values are dynamic).

**Standard Go runtime and process metrics were missing, and that part *was* a real gap.** `internal/observability/metrics.go` builds its own `prometheus.NewRegistry()` (deliberately isolated from `prometheus.DefaultRegisterer`, so tests can construct independent `Metrics` instances without cross-test interference) rather than using the global registry a plain `promhttp.Handler()` setup would default to — and unlike the global registry, a fresh `prometheus.NewRegistry()` does **not** auto-include the Go collector (`go_goroutines`, `go_gc_duration_seconds`, `go_memstats_*`, …) or the process collector (`process_cpu_seconds_total`, `process_resident_memory_bytes`, `process_open_fds`, `process_start_time_seconds`, …) that most Prometheus users expect from any Go service by default. `NewMetrics` now registers `collectors.NewGoCollector()` and `collectors.NewProcessCollector(...)` explicitly (`prometheus/client_golang/prometheus/collectors`), plus `collectors.NewDBStatsCollector(sqlDB, "primary")` when a `*gorm.DB` is supplied (open/in-use/idle connection counts, wait count/duration off `database/sql`'s own `DBStats`) — all three appear on the very first scrape, even with zero application traffic. `NewMetrics` gained a `db *gorm.DB` parameter for this (nil-safe: a nil `db`, or one whose `.DB()` errors, just skips the DB-stats collector rather than failing construction) — `cmd/server/wire_gen.go` passes the already-constructed `db` in, no reordering needed since it's built before `NewMetrics` is called.
