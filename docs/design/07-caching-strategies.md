...
### Round 2: console UI for cache config + embedding settings

Closes the console-UI gap the previous bullet called out.

- **`Keys.tsx` gained a "Response cache" field group** on the create form: `exactEnabled`/`semanticEnabled` checkboxes, `ttlSec`/`semanticThreshold`/`semanticTtlSec` number inputs, and a `billingPolicy` select (`free`/`discount`/`full`, with a conditional `discountPercent` field when `discount` is chosen) — serialized straight into the same `cacheConfig` JSON field the admin API already accepted. No new endpoint; this is purely a form that fills in a field the console previously had no UI for at all.
- **`Settings.tsx` gained a "Semantic cache embedding" section** — a provider select (reusing the already-fetched `Provider[]` list), a model text input, and a dimension number input — following the exact same single-object `GET`/`PUT /ai/gateway/settings` pattern the pre-existing alert-webhook field already used. Same story: no backend change, the fields (`cacheEmbeddingProviderId`/`cacheEmbeddingModel`/`cacheEmbeddingDim`) were already in `SettingsResp`/`UpdateSettingsReq`, just never exposed in a form.
- **Per-key PII/guardrail policy binding** (`AIVirtualKey.PIIPolicyID` — unrelated to caching but shipped in the same pass since it needed the same "add a select to the Keys form" work): a "Guardrail policy" select on the Keys create form, sourced from the new `GET /ai/gateway/pii-policies` endpoint ([D06 round 3](06-security-and-guardrails.md)). Before this, a key could only ever use whatever policy `resolvePIIPolicy` picked as default — there was no way to bind a specific non-default policy to a specific key from the console (or, previously, from any admin endpoint at all).
