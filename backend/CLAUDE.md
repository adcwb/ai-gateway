...
| `biz/protocol_bedrock.go` + `biz/protocol_bedrock_{titan,llama,mistral,nova}.go` + `biz/bedrock/` | outbound bedrock dialect, five model families dispatched by `bedrockModelFamily`: SigV4 signing + AWS event-stream framing (dependency-free package, mirrors `mcpgw`); Claude reuses `openAIToAnthropicRequest`/`anthropicToOpenAIResponse`/`translateAnthropicStream` unchanged, the other four each have their own request/response/stream shape |
...
| `biz/protocol_responses.go` + `biz/responses_api.go` + `biz/responses_state.go` | inbound OpenAI Responses API codec (same wrapper pattern as the Anthropic Messages codec) + `previous_response_id`/`store` server-side conversation persistence (`AIResponseState`, TTL sweeper) |
...
## Model mapping admin API (D01)

`AIModelMapping` (virtual model name → real model + `fallback_chain`, resolved on the hot path by `resolveModelMapping`/`matchModelMapping`) had no admin CRUD before this — only the resolver. `internal/biz/model_mapping_admin.go` (mirrors `mcp_admin.go`) adds Create/List(by `virtual_key_id`, preloading `RealModel`)/Update/Delete at `/ai/gateway/model-mappings`; a duplicate `(virtualKeyId, virtualModel)` is rejected via the existing `uk_vk_virtual_model` unique index. Console: `ModelMappings.tsx` — a key selector, a mapping table, and a create/edit form whose fallback chain is a `@dnd-kit/sortable` reorderable list of `{providerId, model}` rows.

## Pluggable guardrail pipeline + audit encryption (D06)

`internal/biz/guardrail/` (`checker.go`, `chain.go`, `external.go`) is dependency-free w.r.t. `biz` — `Checker` interface, `Chain` (ordered sync run with per-chain deadline + fail-open/closed, most-severe-action-wins, redact rewrites text for later checkers, block/terminate short-circuits; async checkers dispatched without blocking) and the gRPC `external` checker (`api/guardrail/v1/guardrail.proto`, generated `guardrail.pb.go`/`guardrail_grpc.pb.go`) all live there. `biz` depends on `guardrail`, never the reverse: `internal/biz/pii_rules_checker.go` adapts the existing `scanPII` engine as the `pii_rules` built-in checker; `internal/biz/prompt_injection_checker.go`/`internal/biz/topic_fence_checker.go` are two more standalone built-ins (usable in a `checker_chain` without `pii_rules` too) — `prompt_injection` reuses the same `promptInjectionSignatures` list as the legacy `pii_rules.injection` flag (one list, two entry points); `topic_fence` is a curated case-insensitive substring blocklist (`blockedTopics`), not the embedding-similarity mode D06 originally sketched — a documented scope simplification, same "rule-grade, not semantic-grade" honesty `pii_rules` already carries. Both apply the chain-wide `policy.Action` uniformly, like `pii_rules_checker`'s `fixedAction`.

`AIPIIPolicy` gained `checker_chain json` + `fail_mode varchar(8)` (additive, table not renamed). `applyPIIPolicy` (`pii.go`) tries `buildChainForPolicy` first; a policy with no `checker_chain` configured falls through to the exact original single-engine `scanPII` behavior — the pipeline is opt-in per policy, zero behavior change until an operator configures a chain. `applyOutboundGuardrail` (also `pii.go`) mirrors the chain against the assistant's reply for non-streaming responses (identity + translated anthropic/gemini/bedrock dialects); `gateway.go`'s non-streaming response path was restructured (`WriteHeader` moved after the guardrail check, `Content-Length` stripped) so a block/redact finding can still change the committed bytes. **Streaming outbound scanning** (`guardrail_stream.go`'s `guardrailStreamWriter`) now covers the streaming paths too — a wrapper-around-`http.ResponseWriter` (same shape as `anthropicResponseWriter`/`responsesResponseWriter`) that re-scans the accumulated visible assistant text (parsed back out of each SSE chunk, capped at 64 KiB) on every write; `streamProxy`/every `translate*Stream` function are untouched — `gateway.go`'s two streaming call sites (`wrapStreamForGuardrail`/`foldGuardrailStreamVerdict`) just wrap `w` and fold the verdict back into the same `piiAuditCtxKey` side-channel the non-streaming path already populates. A block/redact-worthy finding terminates the stream (further chunks silently swallowed — already-sent bytes can never be rewritten); a log-only finding just records and the stream continues. The upstream read loop isn't cut short on termination (documented minor inefficiency, not a security gap).
...
`AIPIIPolicy` admin CRUD (`internal/biz/pii_policy_admin.go`, mirrors `mcp_admin.go`) lives at `/ai/gateway/pii-policies` — the resolver-only surface (`resolvePIIPolicy`, `buildChainForPolicy`) had no way to create/edit a policy before this. `ListPIIPolicies` populates the previously-unused `BoundKeyCount` field via a `COUNT(*) ... GROUP BY pii_policy_id` on `ai_virtual_keys`. Create/Update enforce **at most one `isDefault=true` policy** in a DB transaction (unset every other row's flag first) — `resolvePIIPolicy`'s fallback query assumes exactly one default and had no guard against an operator creating two, which would make "which policy is default" silently arbitrary. Console: `GuardrailPolicies.tsx` (CRUD table + an ordered add/remove — not drag-sortable — `checker_chain` builder: `pii_rules` detector grid, `prompt_injection` presence-only, `topic_fence` blocked-topics tag input, `external` target+timeout).
...
Both caches are opt-in per key via `AIVirtualKey.CacheConfig` (`keyCacheConfig` in `respcache.go`: `exactEnabled`/`semanticEnabled` + independent TTL/threshold/billing-policy fields), reachable via the admin API (`CreateVirtualKeyReq`/`UpdateVirtualKeyReq.cacheConfig`, raw JSON pass-through) and now the console (`Keys.tsx`'s "Response cache" field group; `Settings.tsx`'s "Semantic cache embedding" section edits `SettingKeyCacheEmbeddingProviderID/Model/Dim` via the existing `GET/PUT /ai/gateway/settings`). Semantic scope (`semanticScopeDigest`) partitions by tenant+resolved-model+generation-params **excluding prompt content**, so paraphrases within the same scope can be found as neighbors; one RediSearch index is created lazily per scope. See `docs/design/07-caching-strategies.md`'s ADR addendum for what's scoped out (cache-flush admin endpoint, live verification against a real RediSearch server — none available here).

## MCP gateway (D09)

`internal/biz/mcpgw/` (dependency-free w.r.t. `biz`, like `guardrail`/`vectorindex`): JSON-RPC 2.0 message shapes + a `Client` forwarding one message to an upstream Streamable HTTP MCP server, plus `ParseBatch` (single-or-batched decode) and `ForwardStream` (GET/SSE passthrough). `internal/biz/mcp_admin.go` (CRUD for `ai_mcp_servers`, mirrors `provider.go`) + `internal/biz/mcp_proxy.go` (`HandleMCPRequest` + `handleOneMCPMessage` + `handleMCPStream`, the governance/forwarding/audit handlers) live in `biz`. Route: `POST/GET/DELETE /ai/mcp/{serverName}`, authenticated by the **same** `middleware.VirtualKeyAuth.ProxyMiddleware` model traffic uses — one credential system, one top-level request-count quota reservation, for both.

Tool governance: `AIVirtualKey.ToolWhitelist` (additive JSON column, exact semantics of `AllowedModels` — empty = unrestricted) gates `tools/call` (JSON-RPC error `-32001` if disallowed, upstream never called) and filters `tools/list` results. Tool calls also draw against a dedicated `AIVirtualKey.HourlyToolCallQuota` (`QuotaDimToolCall`, `QuotaManager.CheckAndReserveToolCall` — same rolling-window Lua script as every other quota, independent of the shared request-count quota; JSON-RPC error `-32003` when exhausted, upstream never called). Tool arguments/results run through the **same D06 guardrail chain** model traffic does (`resolvePIIPolicy` → `buildChainForPolicy` → `guardrail.Chain.Run`) — only activates for policies with `checker_chain` configured (the pluggable path), not the legacy single-engine path. Audit reuses `ai_gateway_audit_logs` (`protocol="mcp"`, `model` column holds `server`, `server/tool`, or `server/stream` for a GET push connection) rather than a parallel table — visible in the console's existing Audit page today. Batched (`[]`) JSON-RPC requests are supported (capped at 20 messages/request, each governed and audited individually via `handleOneMCPMessage`); GET is proxied as a raw byte passthrough of the upstream's SSE push (no per-message governance on that channel — documented gap). Console UI: `frontend/src/pages/McpServers.tsx` (server CRUD) plus tool-whitelist/tool-call-quota fields on the key create form.

Scoped out (see `docs/design/09-extensibility.md` ADR addendum): multi-block tool-result redaction rewrite.

## Hook dispatcher + event bus + WASM sandbox (D09)

Two independent mechanisms, not one — `pre_request`/`post_response` (sync, mutate/reject) go through `internal/biz/extension.Dispatcher`; `on_audit`/`on_billing` generalize directly into `internal/biz/eventbus.Bus` (no Hook interface there, just `Publish(eventType, tenantID, payload)`).

`internal/biz/extension/` (dependency-free w.r.t. `biz`, like `guardrail`): `Dispatcher.RunSync` mirrors `guardrail.Chain.Run` — per-hook deadline (default 100ms), fail-open/closed, panic-contained, first `Reject` short-circuits, `Mutate` patches compose. Three delivery mechanisms per the design doc: compile-time (`Register`/`CompiledHooks` in `registry.go`, wired from `cmd/server/extensions.go`, empty by default), `WebhookHook` (HTTP POST + HMAC-SHA256, `X-AIGW-Signature`), and `WasmHook` (`github.com/tetratelabs/wazero`, one `CompiledModule` per extension, a fresh isolated `api.Module` instantiated per call — modules aren't concurrency-safe). WASM ABI is intentionally minimal and now a real compatibility surface: guest exports `alloc(size u32) -> ptr u32` and `handle(ptr u32, len u32) -> packed u64` (`packed = resultPtr<<32 | resultLen`), JSON in/out, no host-imported callback surface. `internal/biz/extension/wasm_test.go` exercises this against a hand-assembled minimal WASM binary (no `wat2wasm`/TinyGo toolchain available in this environment) — proves the host-side plumbing, not a realistic guest.

`internal/biz/eventbus/` (dependency-free w.r.t. `biz`, but talks to `gorm`/`data/model` directly like `vectorindex` talks to Redis): `Bus.Publish` is a non-blocking channel send (mirrors `BillingManager.Settle`'s `ledgerQ`) into a batch-insert worker that fills a new durable table, `ai_event_log` (mirrors `AuditWorker.processBatch`); one poller goroutine per `Sink` reads rows after its own `ai_event_cursors` position, delivers a batch, advances the cursor only on success, backs off exponentially on failure (mirrors `esRetryWorker`). Shipped sinks: `WebhookSink` (batched, HMAC-signed) and `KafkaSink` (`github.com/segmentio/kafka-go`, topic per event type), both opt-in via `conf.Extensions` (`AIGW_EVENT_WEBHOOK_URL`/`_SECRET`, `AIGW_KAFKA_BROKERS`). The design doc's "cursoring in Redis" prose is superseded by its own "Data model & config" table (`ai_event_cursors` as a DB table) — this is what actually survives a process restart, verified by `eventbus/bus_test.go`'s `TestBus_CursorSurvivesRestart`.

Wiring: `pre_request` runs in `ProxyRequest` right after the PII/guardrail check; `post_response` mutate-capable calls sit at the same two non-streaming call sites as `applyOutboundGuardrail`, plus one terminal annotate-only call for streaming (`runTerminalPostResponseHook` — a `Mutate` attempt there is logged and ignored, bytes are already on the wire). Hook labels flow through a new `hookLabelsCtxKey` side-channel (mirrors `reasoningTokensCtxKey`) into a new additive `AIGatewayAuditLog.HookLabels` column. `on_audit` publishes from `writeAuditLog` right before `uc.audit.Enqueue`; `on_billing` publishes from `BillingManager.applyLedger` right after the ledger transaction commits (never on an idempotent replay). `GatewayUseCase.SetHookDispatcher`/`SetEventBus` and `BillingManager.SetEventBus` are post-construction setters (like `vectorIndex`), not constructor params — both nil-safe, avoiding a mechanical edit of every existing test file that builds a `GatewayUseCase` positionally.

Admin API: `ai_extensions` CRUD (`internal/biz/extension_admin.go`, mirrors `mcp_admin.go`) at `/ai/gateway/extensions`; Create/Update/Delete call `reloadHookDispatcher` to hot-reload without a restart. Reference kit: `examples/extensions/webhook-logger/` (its own standalone Go module — repo root has no `go.mod`).

Scoped out (see `docs/design/09-extensibility.md` ADR addendum): console UI for `ai_extensions` (API-only), breaker-transition/quota-trigger events on the event bus (only `on_audit`/`on_billing` were requested).
...
**Responses API** (`protocol_responses.go` + `responses_api.go` + `responses_state.go`): `POST /ai/v1/responses` (registered before the `/ai/v1/` catch-all, same pattern as `/ai/v1/models`), same wrapper-around-`ProxyRequest` design as Anthropic Messages. `previous_response_id`/`store:true` are supported via server-side conversation persistence: `AIResponseState` (`ai_gateway_responses`, additive) stores one turn's full OpenAI-shape message history — messages actually sent upstream plus the resulting assistant turn — keyed by a response ID minted up front in `ProxyResponses` (`"resp_"+generateRequestID()`, replacing the old non-unique `"resp_"+requestedModel` streaming ID). `previous_response_id` resolution (`loadResponseState`) is scoped to the requesting virtual key — not-found/expired/wrong-key all return the same `PREVIOUS_RESPONSE_NOT_FOUND` (no enumeration signal). `StartResponsesStateSweeper` (ticker, mirrors `StartQuotaReleaseSweeper`) is the only lifecycle control (`conf.ai.responses_state_ttl_hours`, default 24h) — no admin/console visibility, plaintext JSON (no dedicated encryption). Streaming event coverage is scoped to `response.created`/`response.output_text.delta`/`response.function_call_arguments.delta`/`response.completed` — not the full Responses API event taxonomy.

**Bedrock outbound** (`protocol_bedrock.go` + `protocol_bedrock_{titan,llama,mistral,nova}.go` + `internal/biz/bedrock/`, dependency-free w.r.t. `biz` like `mcpgw`/`guardrail`/`vectorindex`): five model families — Claude (`model.ProviderTypeBedrock`'s original scope), Titan, Llama, Mistral, Nova — dispatched by `bedrockModelFamily(modelID)` (substring match, tolerates a cross-region inference-profile prefix like `us.`/`eu.`) at three points: `buildBedrockRequest` (body construction), `bedrockToOpenAIResponse` (sync parse), `translateBedrockStream` (streaming). Claude's original code is unchanged, just extracted into `buildClaudeBedrockRequestBody`/`translateClaudeBedrockStream`. `bedrock.SignRequest` hand-implements AWS SigV4 (no aws-sdk-go dependency; cross-checked in `sigv4_test.go` against an independent Python reference implementation). `bedrock.ReadMessage` decodes the AWS event-stream binary framing every family's `InvokeModelWithResponseStream` uses; a shared `bedrockNextChunk` unwraps the common SDK-level `{"bytes":base64(...)}` envelope on top of that for all five families. Credentials: `AIProvider.APIKey` holds JSON `{"accessKeyId","secretAccessKey","sessionToken"}` for `bedrock`-type providers; region is `adapterConfig.region`. The Claude sync invoke response body IS native Anthropic Messages JSON, so `anthropicToOpenAIResponse` is reused as-is; Titan/Llama/Mistral/Nova each have their own request/response/stream-chunk shape (confirmed against AWS's official Bedrock docs via Context7 — see the D02 ADR addendum for the one unverified point, Nova's native streaming event names) and their own request-body prompt flattening (Llama's `<|begin_of_text|>` chat template, Mistral's `<s>[INST]...[/INST]` — system folded into the first user turn — Titan's `User:`/`Bot:` convention; Nova needs none, its native invoke request is already messages-based). All four new families are text-only in this scope — no tool-calling (no native primitive in three of the four; asymmetric to support only for Nova) and no multimodal.
...
Broad tenant-scoped list filtering (most list endpoints are role-gated only where explicitly named in the RBAC table, not yet tenant-filtered by default), payment gateways/subscriptions/invoices, tool/function calling and multimodal content for the four new Bedrock families (Titan/Llama/Mistral/Nova — text-only in this scope; Claude is unaffected), Mistral's newer chat-completion invoke mode (only the text-completion/`[INST]` mode is supported), Anthropic Message Batches API translation (Batch/Files proxy is openai_compatible-only), ES config wiring (client is nil in wire_gen), `prompt_injection`/`topic_fence` LLM-judge escalation mode (heuristic/keyword-only in this scope) and `topic_fence` embedding-similarity mode (substring blocklist only), external-checker result caching, cache-flush admin endpoint (D07 — TTL is the only invalidation mechanism), console UI for provider `adapter_config`/bedrock credentials and Batch/Files (API-only today), MCP multi-block tool-result redaction rewrite, console UI for `ai_extensions` (API-only today), breaker-transition/quota-trigger events on the event bus (D09 — only `on_audit`/`on_billing` are wired), admin/console visibility into stored Responses API conversations (TTL-only lifecycle, no list/inspect).
